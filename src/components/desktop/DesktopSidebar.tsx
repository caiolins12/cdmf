...
const MASTER_ITEMS: SidebarItem[] = [
  ...
  },
  {
    id: "comunicacoes",
    label: "Comunicações",
    icon: <Ionicons name="chatbubbles-outline" size={18} color="#94A3B8" />,
    iconActive: <Ionicons name="chatbubbles" size={18} color="#7C3AED" />,
  },
  {
    id: "configuracoes",
    label: "Configurações",
    icon: <Ionicons name="settings-outline" size={18} color="#94A3B8" />,
    iconActive: <Ionicons name="settings" size={18} color="#7C3AED" />,
  },
];
...
const DesktopSidebar = React.memo(function DesktopSidebar({
  ...
  // Memoize badge to prevent recalculation on every render
  const badge = useMemo(() => {
    switch (userRole) {
      case "master":
    ...
    <View style={styles.container}>
      ...
      <View style={styles.logoSection}>
        <View style={styles.logoBox}>
          <Image
            source={{ uri: new URL("../../../assets/cdmf-logo.png", import.meta.url).href }}
            style={styles.logo}
            resizeMode="contain"
          />
        </View>
