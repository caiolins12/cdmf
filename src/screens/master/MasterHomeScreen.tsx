...
import { View, Text, StyleSheet, Pressable, ScrollView, RefreshControl, Animated, ActivityIndicator } from "react-native";
...
import { Ionicons, FontAwesome5, MaterialCommunityIcons } from "@/shims/icons";
import { useNavigation, useFocusEffect } from "@react-navigation/native";

const MAX_ACTIVITIES_DISPLAY = 10;

// Componente de card animado (Mobile) - memoized to prevent unnecessary re-renders
const AnimatedStatCard = React.memo(function AnimatedStatCard({ count, label, onPress, isPurple }: {
  count: number;
  label: string;
  onPress: () => void;
  isPurple?: boolean;
...
// Card de estatística compacto para desktop - memoized to prevent unnecessary re-renders
const DesktopStatCard = React.memo(function DesktopStatCard({
  icon,
  iconColor,
  iconBg,
  value,
...
export default function MasterHomeScreen() {
  ...
  const loadStats = async () => {
    try {
      ...
      // Usa totalReceivedNet (valor líquido após taxas) ao invés de totalReceived (valor bruto)
      setMonthlyRevenue(financialSummary.totalReceivedNet || financialSummary.totalReceived);
    } catch (e) {
  ...
  const mobileNavItems = [
    ...
    },
    {
      route: "Comunicacoes",
      title: "Comunicações",
      desc: "WhatsApp e mensagens",
      iconBg: "#FCE7F3",
      icon: <Ionicons name="chatbubbles" size={20} color="#BE185D" />,
    },
  ] as const;
  ...
  return (
    <View style={styles.screen}>
      ...
      <MasterHeader onNavigate={navigateTo} isHome />
      ...
      >
        ...
        <View style={styles.mobileActionsGrid}>
          {mobileNavItems.map((item) => (
            <Pressable
              key={item.route}
              style={styles.mobileActionCard}
              onPress={() => navigation.navigate(item.route as any)}
            >
              <View style={[styles.mobileActionIcon, { backgroundColor: item.iconBg }]}>
                {item.icon}
              </View>
              <Text style={styles.mobileActionTitle}>{item.title}</Text>
              <Text style={styles.mobileActionDesc}>{item.desc}</Text>
            </Pressable>
          ))}
        </View>
        {/* Atividades Recentes */}
