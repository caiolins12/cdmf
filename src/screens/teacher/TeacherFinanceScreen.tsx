...
export default function TeacherFinanceScreen() {
  ...
  const [showTaxModal, setShowTaxModal] = useState(false); // Modal dedicado para taxas do Mercado Pago
  ...
  // Migração automática de transações antigas sem amountNet (executa sempre)
  useEffect(() => {
    const runMigration = async () => {
      if (!isMaster) return;

      try {
        console.log('[Migração Auto] Verificando transações antigas...');
        const { updated, skipped } = await migrateTransactionsAmountNet();

        console.log(`[Migração Auto] Resultado: ${updated} atualizadas, ${skipped} puladas`);

        if (updated > 0) {
          console.log(`[Migração Auto] ${updated} transações atualizadas, recarregando dados...`);
          await loadData();
        }
      } catch (e) {
        console.error('[Migração Auto] Erro ao executar migração:', e);
      }
    };

    // Executa a migração após 2 segundos para não bloquear a UI inicial
    const timer = setTimeout(runMigration, 2000);
    return () => clearTimeout(timer);
  }, [isMaster, migrateTransactionsAmountNet, loadData]);
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      await loadData();
      showAlert("Sucesso", `${created} cobrança(s) criada(s)!`);
    } catch (e: any) {
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      ...
      showAlert("Sucesso", "Pagamento registrado!");
    } catch (e: any) {
      setProcessing(false);
      showAlert("Erro", e.message || "Não foi possível registrar o pagamento");
    }
  };
  ...
  const handleGeneratePix = async (invoice: Invoice) => {
    ...
    try {
      ...
      // Com Mercado Pago ativo, o PIX é gerado pelo aluno no app
      if (USE_MERCADO_PAGO) {
        showAlert(
          "PIX não gerado",
          "O aluno ainda não gerou o código PIX. O código ficará disponível aqui após o aluno acessar a aba de Pagamentos e gerar o PIX pelo app."
        );
        setShowPaymentModal(false);
        return;
      }

      // Gera código PIX local (apenas se Mercado Pago desativado)
      const code = await generatePixCode(invoice);
      setPixCode(code);
      setShowPixModal(true);
    } catch (e: any) {
      showAlert("Erro", e.message || "Configure a chave PIX nas configurações.");
      setShowPaymentModal(false);
    } finally {
      setProcessing(false);
    }
  ...
  const handleCopyPix = async () => {
    ...
    showAlert("Copiado!", "Código PIX copiado");
  };
  ...
  // Calcula prévia das cobranças
  const calculateInvoicePreview = useCallback(async (): Promise<{
    ...
    summary: {
      ...
      byTier: Record<number, { count: number; totalEarly: number; totalRegular: number }>;
      dueDate: string;
      lateDueDate: string;
    };
    ...
    // Filtrar alunos que ainda não têm fatura e estão em turmas
    const studentsWithoutInvoice = students.filter(
      s => !studentsWithInvoice.has(s.uid) && 
      s.enrollmentStatus === "ativo" && 
      s.classes && 
      s.classes.length > 0
    );
    ...
    const byTier: Record<number, { count: number; totalEarly: number; totalRegular: number }> = {};
    ...
    for (const student of studentsWithoutInvoice) {
      // Conta turmas ativas onde o aluno está realmente inscrito
      const activeClasses = allClasses.filter(
        c => c.active && c.studentIds?.includes(student.uid)
      );
      const classCount = Math.max(1, Math.min(activeClasses.length, 4));
      ...
      const discount = isEarlyPayment ? prices.discount : 0;
      
      previews.push({
        student,
        classCount,
        earlyPrice: prices.earlyPrice,
        regularPrice: prices.regularPrice,
        discount,
        currentAmount,
        dueDate,
        lateDueDate,
        hasDiscount: isEarlyPayment && discount > 0,
      });
      
      // Agrupa por tier
      if (!byTier[classCount]) {
        byTier[classCount] = { count: 0, totalEarly: 0, totalRegular: 0 };
      }
      byTier[classCount].count++;
      byTier[classCount].totalEarly += prices.earlyPrice;
      byTier[classCount].totalRegular += prices.regularPrice;
    }
    
    const totalAmount = previews.reduce((sum, p) => sum + p.currentAmount, 0);
    const totalRegularAmount = previews.reduce((sum, p) => sum + p.regularPrice, 0);
    const totalDiscount = totalRegularAmount - totalAmount;
  ...
  }, [month, students, fetchInvoices, getPaymentSettings]);
  ...
  // Carrega prévia quando o modal abre
  useEffect(() => {
    if (showInitialInvoicesModal) {
      setLoadingPreview(true);
      calculateInvoicePreview()
        .then(setInvoicePreview)
        .catch((e) => {
  ...
  }, [showInitialInvoicesModal, calculateInvoicePreview]);
  ...
  const handleBulkDeletePending = async () => {
    ...
    showConfirm(
      "Confirmar",
      `Deseja apagar ${pendingInvoicesToDelete.length} cobrança(s) pendente(s) de ${selectedStudentIds.length} aluno(s)?\n\nEsta ação não pode ser desfeita.`,
      () => executeBulkDelete(pendingInvoicesToDelete)
    );
  };
  ...
  const handleRemoveStudentInvoice = (invoiceId: string) => {
    showConfirm("Confirmar", "Tem certeza que deseja remover esta cobrança?", async () => {
      setProcessing(true);
      try {
  ...
  const handleOpenTaxModal = async () => {
    try {
      ...
      console.error("Erro ao carregar configurações de taxas:", e);
      // Ainda abre o modal mesmo se houver erro, mas paymentSettings pode estar null
      setShowTaxModal(true);
    }
  };
  ...
  const handleSaveTaxSettings = async () => {
    ...
    try {
      setProcessing(true);
      await updatePaymentSettings({
        mercadoPagoTaxPix: paymentSettings.mercadoPagoTaxPix,
        mercadoPagoTaxCard: paymentSettings.mercadoPagoTaxCard,
        mercadoPagoTaxOther: paymentSettings.mercadoPagoTaxOther,
      });
      showAlert("Sucesso", "Taxas do Mercado Pago atualizadas com sucesso!");
      setShowSettingsModal(false);
    } catch (e: any) {
      showAlert("Erro", e.message || "Erro ao salvar configurações de taxas");
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <View style={[styles.screen, isDesktopMode && dkStyles.screen]}>
      ...
      >
        <View style={styles.newInvoiceOverlay}>
          <Pressable style={styles.newInvoiceBackdrop} onPress={() => !processing && setShowCreateInvoiceModal(false)} />
          <View style={styles.newInvoiceContainer}>
            {/* Header compacto */}
            <View style={styles.niHeader}>
              <View style={styles.niHeaderIcon}>
                <Ionicons name="receipt-outline" size={17} color={colors.purple} />
              </View>
              <Text style={styles.niTitle}>Nova Cobrança</Text>
              <Pressable style={styles.niCloseBtn} onPress={() => setShowCreateInvoiceModal(false)}>
                <Ionicons name="close" size={18} color="#64748B" />
              </Pressable>
            </View>

            {/* Body sem scroll externo */}
            <View style={styles.niBody}>
              {/* Tipo */}
              <View style={styles.niSection}>
                <Text style={styles.niSectionTitle}>Tipo</Text>
                <View style={styles.niTypeRow}>
                  {([
                    ...
                    <Pressable
                      key={type.id}
                      style={[styles.niTypeBtn, invoiceType === type.id && styles.niTypeBtnActive]}
                      onPress={() => setInvoiceType(type.id)}
                    >
                      <Ionicons name={type.icon as any} size={14} color={invoiceType === type.id ? "#fff" : "#64748B"} />
                      <Text style={[styles.niTypeBtnText, invoiceType === type.id && styles.niTypeBtnTextActive]}>{type.label}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
              ...
              {/* Alunos */}
              <View style={styles.niSection}>
                <View style={styles.niSectionHeaderRow}>
                  <Text style={styles.niSectionTitle}>Alunos</Text>
                  <View style={styles.niFilterRow}>
                    {([
                      ...
                      <Pressable
                        key={f.id}
                        style={[styles.niFilterBtn, studentFilter === f.id && styles.niFilterBtnActive]}
                        onPress={() => setStudentFilter(f.id)}
                      >
                        <Text style={[styles.niFilterText, studentFilter === f.id && styles.niFilterTextActive]}>{f.label}</Text>
                      </Pressable>
                    ))}
                  </View>
                  <Pressable style={styles.niSelectAllBtn} onPress={areAllFilteredSelected ? deselectAllStudents : selectAllStudents}>
                    <Ionicons name={areAllFilteredSelected ? "close-circle" : "checkmark-done-circle"} size={14} color={colors.purple} />
                    <Text style={styles.niSelectAllText}>{areAllFilteredSelected ? "Limpar" : "Todos"}</Text>
                  </Pressable>
                </View>
                <ScrollView style={styles.niChipScroll} contentContainerStyle={styles.niChips} showsVerticalScrollIndicator={false} nestedScrollEnabled>
                  {getFilteredStudents().map(s => {
                    const isSelected = newInvoiceStudentIds.includes(s.uid);
                    return (
                      <Pressable
                        key={s.uid}
                        style={[styles.niStudentChip, isSelected && styles.niStudentChipSelected]}
                        onPress={() => toggleStudentSelection(s.uid)}
                      >
                        <Ionicons name={isSelected ? "checkmark-circle" : "ellipse-outline"} size={13} color={isSelected ? "#fff" : "#94A3B8"} />
                        <Text style={[styles.niStudentName, isSelected && styles.niStudentNameSelected]}>{s.name.split(" ")[0]}</Text>
                      </Pressable>
                    );
                  })}
                  {getFilteredStudents().length === 0 && (
                    <Text style={styles.niEmptyText}>Nenhum aluno encontrado</Text>
                  )}
                </ScrollView>
                {newInvoiceStudentIds.length > 0 && (
                  <Text style={styles.niSelectedCount}>{newInvoiceStudentIds.length} selecionado{newInvoiceStudentIds.length > 1 ? "s" : ""}</Text>
                )}
              </View>

              {/* Detalhes */}
              <View style={styles.niSection}>
                <Text style={styles.niSectionTitle}>Detalhes</Text>
                <View style={styles.niFieldsRow}>
                  <View style={styles.niField}>
                    <Text style={styles.niFieldLabel}>Valor</Text>
                    <View style={styles.niValueBox}>
                      <Text style={styles.niValuePrefix}>R$</Text>
                      <TextInput
                        id="new-invoice-amount"
                        name="new-invoice-amount"
                        style={styles.niValueInput}
                  ...
                  <View style={styles.niField}>
                    <Text style={styles.niFieldLabel}>Vencimento</Text>
                    <TextInput
                      id="new-invoice-due-date"
                      name="new-invoice-due-date"
                      style={styles.niTextInput}
                ...
                <Text style={styles.niFieldLabel}>Descrição</Text>
                <TextInput
            ...
            {/* Footer */}
            <View style={styles.niFooter}>
              <Pressable style={styles.niCancelBtn} onPress={() => setShowCreateInvoiceModal(false)}>
                <Text style={styles.niCancelText}>Cancelar</Text>
              </Pressable>
              <Pressable
                style={[styles.niSubmitBtn, processing && { opacity: 0.7 }]}
              ...
              >
                {processing ? (
                  ...
                  <>
                    <Ionicons name="checkmark-circle" size={16} color="#fff" />
                    <Text style={styles.niSubmitText}>Criar Cobrança</Text>
                  </>
                )}
              </Pressable>
            </View>
      ...
      {/* Modal: Registrar Pagamento (Unificado com PIX) */}
      <Modal visible={showPaymentModal || showPixModal} transparent animationType="fade">
              ...
              <>
                ...
                <View style={styles.modalBtns}>
                  <Pressable style={styles.btnSecondary} onPress={async () => {
                    ...
                    <Text style={styles.btnSecondaryText}>Copiar Código</Text>
                  </Pressable>
                  <Pressable 
                    style={[styles.btnPrimary, { backgroundColor: colors.green }]} 
                    onPress={async () => {
                      if (selectedInvoice) {
                ...
                <Pressable style={[styles.btnSecondary, { marginTop: 8, width: "100%" }]} onPress={() => {
                  setShowPixModal(false);
                  setPixCode("");
                }}>
              ...
              <>
                ...
                <View style={styles.methodsGrid}>
                  {([
                    ...
                    { id: "pix_mercadopago", label: "PIX (MP)", icon: "logo-google" },
                    ...
                    <Pressable
                      ...
                      onPress={async () => {
                        ...
                        if ((m.id === "pix" || m.id === "pix_mercadopago") && selectedInvoice) {
                          try {
                            if (selectedInvoice.pixCode && selectedInvoice.mpPaymentId) {
                              setPixCode(selectedInvoice.pixCode);
                              setShowPixModal(true);
                            } else {
                              const code = await generatePixCode(selectedInvoice);
                              setPixCode(code);
                              setShowPixModal(true);
                            }
                          } catch (e: any) {
                ...
                {paymentMethod !== "pix" && paymentMethod !== "pix_mercadopago" && (
                  <>
      ...
      {/* Modal: Gerar Mensalidades - Com Prévia */}
      <Modal visible={showInitialInvoicesModal} transparent animationType="fade">
        <Pressable style={styles.modalOverlay} onPress={() => !processing && setShowInitialInvoicesModal(false)}>
          <View style={styles.gmModal} onStartShouldSetResponder={() => true}>
            ...
            {loadingPreview ? (
              <View style={styles.gmLoading}>
                <ActivityIndicator size="large" color={colors.purple} />
                <Text style={styles.gmLoadingText}>Calculando prévia...</Text>
              </View>
            ) : invoicePreview && invoicePreview.previews.length > 0 ? (
              <>
                {/* Resumo compacto */}
                <View style={styles.gmSummary}>
                  <View style={styles.gmSummaryItem}>
                    <Text style={styles.gmSummaryLabel}>Alunos</Text>
                    <Text style={[styles.gmSummaryValue, { color: colors.purple }]}>{invoicePreview.summary.totalStudents}</Text>
                  </View>
                  <View style={styles.gmSummaryDivider} />
                  <View style={styles.gmSummaryItem}>
                    <Text style={styles.gmSummaryLabel}>Total</Text>
                    <Text style={[styles.gmSummaryValue, { color: "#059669" }]}>{formatCurrency(invoicePreview.summary.totalAmount)}</Text>
                  </View>
                  {invoicePreview.summary.totalDiscount > 0 && (
                    <>
                      <View style={styles.gmSummaryDivider} />
                      <View style={styles.gmSummaryItem}>
                        <Text style={styles.gmSummaryLabel}>Desconto</Text>
                        <Text style={[styles.gmSummaryValue, { color: "#DC2626" }]}>{formatCurrency(invoicePreview.summary.totalDiscount)}</Text>
                      </View>
                    </>
                  )}
                </View>
                ...
                {/* Datas lado a lado */}
                <View style={styles.gmDatesRow}>
                  <View style={styles.gmDateItem}>
                    <View style={[styles.gmDateDot, { backgroundColor: "#059669" }]} />
                    <Text style={styles.gmDateLabel}>Desconto até</Text>
                    <Text style={[styles.gmDateValue, { color: "#059669" }]}>{formatDateSafe(invoicePreview.summary.dueDate)}</Text>
                  </View>
                  <View style={styles.gmDateItem}>
                    <View style={[styles.gmDateDot, { backgroundColor: "#DC2626" }]} />
                    <Text style={styles.gmDateLabel}>Vencimento</Text>
                    <Text style={[styles.gmDateValue, { color: "#DC2626" }]}>{formatDateSafe(invoicePreview.summary.lateDueDate)}</Text>
                  </View>
                </View>

                {/* Tabela de preços (compacta) */}
                {invoicePreview.settings && invoicePreview.settings.tierPricing && (
                  <View style={styles.gmPricingTable}>
                    <View style={styles.gmPricingHeader}>
                      <Text style={[styles.gmPricingCell, { flex: 0.8 }]}>Turmas</Text>
                      <Text style={styles.gmPricingCell}>Com Desc.</Text>
                      <Text style={styles.gmPricingCell}>Normal</Text>
                      <Text style={styles.gmPricingCell}>Alunos</Text>
                    </View>
                    {Object.entries(invoicePreview.summary.byTier)
                      .sort(([a], [b]) => Number(a) - Number(b))
                      .map(([tier, data]) => (
                        <View key={tier} style={styles.gmPricingRow}>
                          <Text style={[styles.gmPricingCell, { flex: 0.8 }]}>{tier}×</Text>
                          <Text style={[styles.gmPricingCell, { color: "#059669", fontWeight: "600" }]}>{formatCurrency(data.totalEarly)}</Text>
                          <Text style={styles.gmPricingCell}>{formatCurrency(data.totalRegular)}</Text>
                          <Text style={[styles.gmPricingCell, { fontWeight: "600" }]}>{data.count}</Text>
                        </View>
                      ))}
                  </View>
                )}
                ...
                {/* Cabeçalho da lista */}
                <View style={styles.gmStudentHeader}>
                  <Ionicons name="people" size={13} color={colors.purple} />
                  <Text style={styles.gmStudentHeaderText}>Alunos que receberão cobrança</Text>
                </View>

                {/* Lista de alunos — scroll interno */}
                <ScrollView style={styles.gmStudentScroll} showsVerticalScrollIndicator={false} nestedScrollEnabled>
                  {invoicePreview.previews.map((preview) => (
                    <View key={preview.student.uid} style={styles.gmStudentItem}>
                      <Text style={styles.gmStudentName} numberOfLines={1}>{preview.student.name}</Text>
                      <View style={styles.gmStudentMeta}>
                        <View style={styles.gmStudentBadge}>
                          <Text style={styles.gmStudentBadgeText}>{preview.classCount} turma{preview.classCount > 1 ? "s" : ""}</Text>
                        </View>
                        {preview.hasDiscount && (
                          <View style={[styles.gmStudentBadge, { backgroundColor: "#D1FAE5" }]}>
                            <Text style={[styles.gmStudentBadgeText, { color: "#059669" }]}>-{formatCurrency(preview.discount)}</Text>
                          </View>
                        )}
                      </View>
                      <Text style={[styles.gmStudentAmount, { color: preview.hasDiscount ? "#059669" : "#1E293B" }]}>
              ...
              <View style={styles.gmEmpty}>
                <Ionicons name="checkmark-done-circle" size={44} color="#059669" />
                <Text style={styles.gmEmptyTitle}>Todos em dia</Text>
                <Text style={styles.gmEmptySubtitle}>Não há alunos sem cobrança para {formatMonthDisplay(month)}</Text>
              </View>
            )}

            {/* Footer com botões melhorados */}
            <View style={styles.gmFooter}>
              <Pressable
                style={styles.gmCancelBtn}
                onPress={() => { setShowInitialInvoicesModal(false); setInvoicePreview(null); }}
                disabled={processing}
              >
                <Ionicons name="close-circle-outline" size={16} color="#64748B" />
                <Text style={styles.gmCancelText}>Cancelar</Text>
              </Pressable>
              <Pressable
                style={[styles.gmGenerateBtn, (!invoicePreview || invoicePreview.previews.length === 0) && styles.gmGenerateBtnDisabled]}
                onPress={handleGenerateInitialInvoices}
                disabled={processing || !invoicePreview || invoicePreview.previews.length === 0}
              >
                {processing ? <ActivityIndicator color="#fff" size="small" /> : (
                  <>
                    <Ionicons name="flash" size={16} color="#fff" />
                    <Text style={styles.gmGenerateText}>
                      Gerar {invoicePreview?.summary.totalStudents || 0} Cobrança{(invoicePreview?.summary.totalStudents || 0) !== 1 ? "s" : ""}
                    </Text>
                  </>
                )}
              </Pressable>
      ...
      <Modal visible={showStudentAccountModal} transparent animationType="fade">
        <Pressable style={styles.modalOverlay} onPress={() => !processing && setShowStudentAccountModal(false)}>
          <View style={styles.saModal} onStartShouldSetResponder={() => true}>

            {/* Header compacto */}
            <View style={styles.saHeader}>
              <View style={styles.saAvatar}>
                <Text style={styles.saAvatarText}>
                  {selectedStudentForAccount?.name?.charAt(0).toUpperCase() || "?"}
                </Text>
              </View>
              <View style={styles.saHeaderInfo}>
                <Text style={styles.saHeaderName} numberOfLines={1}>
                  {selectedStudentForAccount?.name || "Aluno"}
                </Text>
                <Text style={styles.saHeaderSub}>Controle de Conta</Text>
              </View>
              <Pressable onPress={() => setShowStudentAccountModal(false)} hitSlop={10}>
                <Ionicons name="close" size={20} color="#64748B" />
              </Pressable>
            </View>
            ...
            <View style={styles.saSection}>
              <View style={styles.saSectionRow}>
                <Text style={styles.saSectionTitle}>Cobranças deste mês</Text>
                {selectedStudentForAccount && (() => {
                  const count = invoices.filter(inv => inv.studentId === selectedStudentForAccount.uid).length;
                  return count > 0 ? (
                    <View style={styles.saSectionBadge}>
                      <Text style={styles.saSectionBadgeText}>{count}</Text>
                    </View>
                  ) : null;
                })()}
              </View>

              {!selectedStudentForAccount || invoices.filter(inv => inv.studentId === selectedStudentForAccount.uid).length === 0 ? (
                <View style={styles.saEmpty}>
                  <Ionicons name="document-outline" size={18} color="#94A3B8" />
                  <Text style={styles.saEmptyText}>Nenhuma cobrança neste mês</Text>
                </View>
              ) : (
                <ScrollView style={styles.saInvoiceScroll} showsVerticalScrollIndicator={false} nestedScrollEnabled>
                  {invoices
                    .filter(inv => inv.studentId === selectedStudentForAccount.uid)
                    .map(inv => (
                      <View key={inv.id} style={styles.saInvoiceRow}>
                        <View style={styles.saInvoiceLeft}>
                          <Text style={styles.saInvoiceDesc} numberOfLines={1}>{inv.description}</Text>
                          <View style={[styles.saStatusChip, {
                            backgroundColor: inv.status === "paid" ? "#D1FAE5" : inv.status === "overdue" ? "#FEE2E2" : "#FEF3C7"
                          }]}>
                            <Text style={[styles.saStatusChipText, {
                        ...
                        <Text style={styles.saInvoiceAmt}>{formatCurrency(inv.amount)}</Text>
                        {inv.status !== "paid" && (
                          <View style={styles.saInvoiceBtns}>
                            <Pressable
                              style={styles.saIconBtn}
                              onPress={() => { setSelectedInvoice(inv); setShowStudentAccountModal(false); handleGeneratePix(inv); }}
                            >
                              <Ionicons name="qr-code-outline" size={15} color={colors.purple} />
                            </Pressable>
                            <Pressable
                              style={[styles.saIconBtn, { backgroundColor: "#F0FDF4", borderColor: "#BBF7D0" }]}
                              onPress={() => { setSelectedInvoice(inv); setShowStudentAccountModal(false); openPaymentModal(inv); }}
                            >
                              <Ionicons name="checkmark-circle-outline" size={15} color={colors.green} />
                            </Pressable>
                            <Pressable
                              style={[styles.saIconBtn, { backgroundColor: "#FFF5F5", borderColor: "#FED7D7" }]}
            ...
            <View style={styles.saSection}>
              <View style={styles.saSectionRow}>
                <Text style={styles.saSectionTitle}>Adicionar Cobrança</Text>
              </View>

              {/* Mensalidade padrão */}
              <Pressable style={styles.saStandardBtn} onPress={handleAddStandardInvoice} disabled={processing}>
                <View style={styles.saStandardIcon}>
                  <Ionicons name="receipt" size={17} color="#16A34A" />
                </View>
                <View style={styles.saStandardInfo}>
                  <Text style={styles.saStandardTitle}>Mensalidade Padrão</Text>
                  <Text style={styles.saStandardSub}>Calculado pelas turmas</Text>
                </View>
                {processing ? <ActivityIndicator size="small" color="#16A34A" /> : <Ionicons name="add-circle" size={22} color="#16A34A" />}
              </Pressable>

              {/* Cobrança personalizada compacta */}
              <View style={styles.saCustomArea}>
                <Text style={styles.saCustomLabel}>Personalizada</Text>
                <View style={styles.saCustomFields}>
                  <TextInput
                    id="custom-invoice-amount"
                    name="custom-invoice-amount"
                    style={styles.saAmountInput}
                  ...
                  />
                  <Pressable
                    style={[styles.saCustomAddBtn, (!customInvoiceAmount || processing) && { opacity: 0.5 }]}
                    onPress={handleAddCustomInvoice}
                    disabled={processing || !customInvoiceAmount}
                  >
                    {processing ? <ActivityIndicator color="#fff" size="small" /> : <Ionicons name="add" size={18} color="#fff" />}
                  </Pressable>
                </View>
              </View>
            </View>
            ...
            {/* Footer */}
            <View style={styles.saFooter}>
              <Pressable style={styles.saCloseBtn} onPress={() => setShowStudentAccountModal(false)}>
                <Text style={styles.saCloseBtnText}>Fechar</Text>
              </Pressable>
            </View>
          </View>
        </Pressable>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.vmModal}>
            ...
            <View style={styles.vmHeader}>
              <View style={styles.vmHeaderIcon}>
                <Ionicons name="pricetag" size={16} color="#059669" />
              </View>
              <Text style={styles.vmTitle}>Valores e Descontos</Text>
              <Pressable onPress={() => setShowPricingModal(false)}>
                <Ionicons name="close" size={20} color="#64748B" />
              </Pressable>
            </View>

            {/* Corpo — scroll apenas se necessário */}
            <ScrollView
              style={styles.vmBody}
              contentContainerStyle={styles.vmBodyContent}
              keyboardShouldPersistTaps="handled"
              showsVerticalScrollIndicator={false}
            >
              {/* Prazos de pagamento */}
              <View style={styles.vmDeadlinesSection}>
                <Text style={styles.vmSectionTitle}>Prazos de Pagamento</Text>
                <View style={styles.vmDeadlinesRow}>
                  <View style={styles.vmDeadlineField}>
                    <Text style={styles.vmDeadlineLabel}>Desconto até dia</Text>
                    <TextInput
                      id="settings-due-day"
                      name="settings-due-day"
                      style={styles.vmDeadlineInput}
                  ...
                  <View style={styles.vmDeadlineField}>
                    <Text style={styles.vmDeadlineLabel}>Vencimento dia</Text>
                    <TextInput
                      id="settings-late-deadline"
                      name="settings-late-deadline"
                      style={styles.vmDeadlineInput}
                ...
                <View style={styles.vmHint}>
                  <Ionicons name="information-circle-outline" size={12} color="#94A3B8" />
                  <Text style={styles.vmHintText}>
                    Desconto até dia {settingsDueDayOfMonth}. Atrasado após dia {settingsLatePaymentDeadline}.
                  </Text>
                </View>
              </View>

              {/* Divisor */}
              <View style={styles.vmDivider} />

              {/* Tabela de Preços */}
              <View style={styles.vmPricingSection}>
                <Text style={styles.vmSectionTitle}>Tabela de Preços por Turmas</Text>
                <View style={styles.vmPricingTable}>
                  <View style={styles.vmPricingHeader}>
                    <Text style={[styles.vmPricingCell, { flex: 0.5 }]}>Turmas</Text>
                    <Text style={styles.vmPricingCell}>Com Desconto</Text>
                    <Text style={styles.vmPricingCell}>Valor Normal</Text>
                  </View>
                  {tierPricing.map((tier, index) => (
                    <View key={tier.classes} style={styles.vmPricingRow}>
                      <View style={[styles.vmPricingCellView, { flex: 0.5 }]}>
                        <View style={styles.vmTierBadge}>
                          <Text style={styles.vmTierBadgeText}>{tier.classes}×</Text>
                        </View>
                      </View>
                      <View style={styles.vmPricingCellView}>
                        <View style={styles.vmPriceInputBox}>
                          <Text style={styles.vmPricePrefix}>R$</Text>
                          <TextInput
                            id={`price-early-${index}`}
                            name={`price-early-${index}`}
                            style={styles.vmPriceInput}
            ...
            <View style={styles.vmFooter}>
              <Pressable style={styles.vmCancelBtn} onPress={() => setShowPricingModal(false)}>
                <Ionicons name="close-circle-outline" size={16} color="#64748B" />
                <Text style={styles.vmCancelText}>Cancelar</Text>
              </Pressable>
              <Pressable style={styles.vmSaveBtn} onPress={handleSavePricingSettings} disabled={processing}>
                {processing ? <ActivityIndicator color="#fff" size="small" /> : (
                  <>
                    <Ionicons name="checkmark-circle" size={16} color="#fff" />
                    <Text style={styles.vmSaveText}>Salvar Valores</Text>
                  </>
                )}
              </Pressable>
            </View>
      ...
      <Modal visible={showSettingsModal} transparent animationType="fade">
        <Pressable style={styles.modalOverlay} onPress={() => setShowSettingsModal(false)}>
          <Pressable style={[styles.modal, styles.settingsModal, isDesktopMode && dkStyles.settingsModal]} onPress={e => e.stopPropagation()}>
            <View style={[styles.modalHeader, { paddingTop: 20, paddingHorizontal: 16, paddingBottom: 0 }]}>
      ...
      {/* Modal: Taxas do Mercado Pago */}
      <Modal visible={showTaxModal} transparent animationType="fade">
        <Pressable style={styles.modalOverlay} onPress={() => setShowTaxModal(false)}>
          <Pressable style={[styles.modal, styles.settingsModal, isDesktopMode && dkStyles.settingsModal]} onPress={e => e.stopPropagation()}>
            <View style={[styles.modalHeader, { paddingTop: 20, paddingHorizontal: 16, paddingBottom: 0 }]}>
              <Ionicons name="receipt-outline" size={32} color={colors.purple} />
              <Text style={styles.modalTitle}>Taxas do Mercado Pago</Text>
              <Text style={styles.modalSubtitle}>Configure as taxas de transação</Text>
            </View>
            ...
            <ScrollView 
              ...
              bounces={false}
            >
              {isMaster && paymentSettings ? (
                <View style={styles.settingsSection}>
                  <Text style={styles.settingsSectionDesc}>
                    Configure as taxas cobradas pelo Mercado Pago. Os valores líquidos (após dedução de taxas) 
                    serão exibidos como o saldo disponível na conta.
                  </Text>
                  
                  <View style={styles.taxSettingsGrid}>
                    <View style={styles.taxSettingItem}>
                      <Text style={styles.taxSettingLabel}>Taxa PIX (%)</Text>
                      <TextInput
                        id="tax-pix"
                        name="tax-pix"
                        style={styles.taxInput}
                        value={paymentSettings.mercadoPagoTaxPix?.toFixed(2) || "1.00"}
                        onChangeText={(text) => {
                          const value = parseFloat(text.replace(",", ".")) || 0;
                          setPaymentSettings({ ...paymentSettings, mercadoPagoTaxPix: value });
                        }}
                        keyboardType="decimal-pad"
                        placeholder="1.00"
                      />
                      <Text style={styles.taxSettingHint}>Taxa para pagamentos via PIX</Text>
                    </View>
                    
                    <View style={styles.taxSettingItem}>
                      <Text style={styles.taxSettingLabel}>Taxa Cartão (%)</Text>
                      <TextInput
                        id="tax-card"
                        name="tax-card"
                        style={styles.taxInput}
                        value={paymentSettings.mercadoPagoTaxCard?.toFixed(2) || "4.99"}
                        onChangeText={(text) => {
                          const value = parseFloat(text.replace(",", ".")) || 0;
                          setPaymentSettings({ ...paymentSettings, mercadoPagoTaxCard: value });
                        }}
                        keyboardType="decimal-pad"
                        placeholder="4.99"
                      />
                      <Text style={styles.taxSettingHint}>Taxa para pagamentos via cartão</Text>
                    </View>
                    
                    <View style={styles.taxSettingItem}>
                      <Text style={styles.taxSettingLabel}>Taxa Outros (%)</Text>
                      <TextInput
                        id="tax-other"
                        name="tax-other"
                        style={styles.taxInput}
                        value={paymentSettings.mercadoPagoTaxOther?.toFixed(2) || "1.00"}
                        onChangeText={(text) => {
                          const value = parseFloat(text.replace(",", ".")) || 0;
                          setPaymentSettings({ ...paymentSettings, mercadoPagoTaxOther: value });
                        }}
                        keyboardType="decimal-pad"
                        placeholder="1.00"
                      />
                      <Text style={styles.taxSettingHint}>Taxa para outros métodos de pagamento</Text>
                    </View>
                  </View>
                  
                  <Pressable 
                    style={styles.saveTaxBtn} 
                    onPress={async () => {
                      await handleSaveTaxSettings();
                  ...
                  >
                    {processing ? <ActivityIndicator color="#fff" size="small" /> : (
                      <>
                        ...
                        <Text style={styles.saveTaxBtnText}>Salvar Taxas</Text>
                      </>
                    )}
                  </Pressable>
                </View>
            ...
            <View style={styles.settingsModalBtns}>
              <Pressable style={styles.settingsCancelBtn} onPress={() => setShowTaxModal(false)}>
                <Text style={styles.settingsCancelBtnText}>Fechar</Text>
              </Pressable>
            </View>
          </Pressable>
      ...
      <ScrollView 
        ...
        showsVerticalScrollIndicator={true}
        refreshControl={!isDesktopMode ? <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[colors.purple]} /> : undefined}
      >
        ...
        <View style={[styles.statsGrid, isDesktopMode && dkStyles.statsGrid]}>
          <StatCard 
            ...
            subtitle={summary?.totalFees && summary.totalFees > 0 ? `Líquido: ${formatCurrency(summary.totalReceivedNet || summary.totalReceived || 0)} - Taxas: ${formatCurrency(summary.totalFees)}` : "Valor líquido na conta"}
            icon="wallet" 
            color={colors.purple} 
            bgColor="#EDE9FE" 
          />
          <StatCard 
            ...
            value={formatCurrency(summary?.totalReceivedNet || summary?.totalReceived || 0)} 
            subtitle={`${summary?.totalReceived ? `${formatCurrency(summary.totalReceived)} bruto` : ''}${summary?.totalFees && summary.totalFees > 0 ? ` - ${formatCurrency(summary.totalFees)} taxa` : ''}`}
            icon="trending-up" 
            color="#059669" 
            bgColor="#D1FAE5" 
          />
          <StatCard 
            ...
            subtitle={summary?.totalFees && summary.totalFees > 0 ? "Taxas do Mercado Pago" : "Nenhuma taxa aplicada"}
        ...
        {isDesktopMode ? (
          <View style={[styles.actionToolbar, dkStyles.actionToolbar]}>
            ...
            {isMaster && (
              ...
              >
                ...
                <Text style={styles.toolbarBtnText}>Mensalidades</Text>
              </Pressable>
            )}
            ...
            <Pressable
              ...
              disabled={!isMaster}
            >
              <Ionicons name="pricetag" size={14} color={isMaster ? "#059669" : "#94A3B8"} />
            ...
            {isMaster && (
              <Pressable
                style={styles.toolbarBtn}
                onPress={handleOpenTaxModal}
              >
                <Ionicons name="receipt-outline" size={15} color={colors.purple} />
                <Text style={styles.toolbarBtnText}>Taxas MP</Text>
              </Pressable>
            )}
          ...
          <View style={styles.actionToolbarMobile}>
            ...
            {isMaster && (
              <Pressable
                style={[styles.toolbarBtnMobile, styles.toolbarBtnMobileFlash]}
                onPress={() => setShowInitialInvoicesModal(true)}
              >
                <Ionicons name="flash" size={24} color="#D97706" />
                <Text style={[styles.toolbarBtnMobileText, styles.toolbarBtnMobileTextFlash]}>Mensalidades</Text>
              </Pressable>
            )}

            {isMaster && (
              <Pressable
                style={[styles.toolbarBtnMobile, styles.toolbarBtnMobileGreen]}
                onPress={handleOpenPricingSettings}
              >
                <Ionicons name="pricetag" size={22} color="#059669" />
                <Text style={[styles.toolbarBtnMobileText, styles.toolbarBtnMobileTextGreen]}>Valores</Text>
              </Pressable>
            )}
          ...
          <View style={[styles.section, isDesktopMode && dkStyles.section]}>
            ...
            <View style={styles.sectionHeaderNew}>
              <View style={styles.sectionTitleRow}>
                ...
                <Text style={styles.sectionTitleNew}>Gestão Financeira</Text>
                <View style={styles.totalBadge}>
                  <Text style={styles.totalBadgeText}>{enrichedStudents.length} alunos • {invoices.length} faturas</Text>
                </View>
              </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.statusFilterScroll} contentContainerStyle={styles.statusFilterContent}>
                {([
                  { key: "all",     label: "Todos",     count: statusCounts.all,     color: "#64748B", bg: "#F1F5F9" },
                  { key: "pending", label: "Pendentes", count: statusCounts.pending, color: "#D97706", bg: "#FEF3C7" },
                  { key: "overdue", label: "Atrasados", count: statusCounts.overdue, color: "#DC2626", bg: "#FEE2E2" },
                  { key: "paid",    label: "Em dia",    count: statusCounts.paid,    color: "#059669", bg: "#D1FAE5" },
                ] as const).map(tab => {
                  const isActive = statusFilter === tab.key;
                  return (
                    <Pressable
                      key={tab.key}
                      style={[styles.statusFilterChip, isActive && { backgroundColor: tab.color, borderColor: tab.color }]}
                      onPress={() => setStatusFilter(tab.key)}
                    >
                      <View style={[styles.statusFilterDot, { backgroundColor: isActive ? "rgba(255,255,255,0.6)" : tab.color }]} />
                      <Text style={[styles.statusFilterLabel, isActive && { color: "#fff" }]}>{tab.label}</Text>
                      <View style={[styles.statusFilterBadge, { backgroundColor: isActive ? "rgba(255,255,255,0.25)" : tab.bg }]}>
                        <Text style={[styles.statusFilterCount, { color: isActive ? "#fff" : tab.color }]}>{tab.count}</Text>
                      </View>
                    </Pressable>
                  );
                })}
            ...
            {/* Lista Unificada: Alunos (quando "all") ou Faturas (quando filtrado) */}
            {statusFilter === "all" ? (
              /* Lista de Alunos */
              ...
              <>
                {statusFilter === "pending" && (
                  pendingInvoices.filter(inv => inv.status === "pending").length === 0 ? (
                    <View style={styles.emptyStateNew}>
                      <Ionicons name="checkmark-done-circle" size={48} color="#059669" />
                      <Text style={styles.emptyStateTitle}>Tudo em dia!</Text>
                      <Text style={styles.emptyStateSubtitle}>Nenhuma cobrança pendente</Text>
                    </View>
                  ) : (
                    <View style={styles.cardsList}>
                      {pendingInvoices.filter(inv => inv.status === "pending").slice(0, pendingDisplayLimit).map(inv => (
                        <InvoiceCard key={inv.id} invoice={inv} onPix={() => handleGeneratePix(inv)} onMarkPaid={() => openPaymentModal(inv)} isDesktop={isDesktopMode} />
                      ))}
                      {pendingInvoices.filter(inv => inv.status === "pending").length > pendingDisplayLimit && (
                        <Pressable 
                          style={styles.loadMoreBtnNew}
                        ...
                        >
                          <Ionicons name="chevron-down-circle-outline" size={20} color={colors.purple} />
                          <Text style={styles.loadMoreBtnText}>
                            Carregar mais ({pendingInvoices.filter(inv => inv.status === "pending").length - pendingDisplayLimit} restantes)
                          </Text>
                        </Pressable>
                      )}
                    </View>
                ...
                {statusFilter === "overdue" && (
                  pendingInvoices.filter(inv => inv.status === "overdue").length === 0 ? (
                    <View style={styles.emptyStateNew}>
                      <Ionicons name="checkmark-done-circle" size={48} color="#059669" />
                      <Text style={styles.emptyStateTitle}>Nenhuma cobrança atrasada</Text>
                      <Text style={styles.emptyStateSubtitle}>Tudo em dia!</Text>
                    </View>
                  ) : (
                    <View style={styles.cardsList}>
                      {pendingInvoices.filter(inv => inv.status === "overdue").slice(0, pendingDisplayLimit).map(inv => (
                        <InvoiceCard key={inv.id} invoice={inv} onPix={() => handleGeneratePix(inv)} onMarkPaid={() => openPaymentModal(inv)} isDesktop={isDesktopMode} />
                      ))}
                      {pendingInvoices.filter(inv => inv.status === "overdue").length > pendingDisplayLimit && (
                        <Pressable 
                          style={styles.loadMoreBtnNew}
                          onPress={() => setPendingDisplayLimit(prev => prev + ITEMS_PER_PAGE)}
                        >
                          <Ionicons name="chevron-down-circle-outline" size={20} color={colors.purple} />
                          <Text style={styles.loadMoreBtnText}>
                            Carregar mais ({pendingInvoices.filter(inv => inv.status === "overdue").length - pendingDisplayLimit} restantes)
                          </Text>
                        </Pressable>
                      )}
                    </View>
                ...
                {statusFilter === "paid" && (
                  paidInvoices.length === 0 ? (
                    <View style={styles.emptyStateNew}>
                      ...
                      <Text style={styles.emptyStateTitle}>Nenhum pagamento ainda</Text>
                      <Text style={styles.emptyStateSubtitle}>Pagamentos confirmados aparecerão aqui</Text>
                    </View>
                  ) : (
                    <>
...
const styles = StyleSheet.create({
  ...
  toolbarBtnPrimaryText: { fontSize: 13, fontWeight: "700", color: "#fff" },
  toolbarBtn: {
    flexDirection: "row",
    alignItems: "center",
    gap: 5,
  ...
  toolbarBtnMobileFlash: {
    backgroundColor: "#FFFBEB",
    borderColor: "#FDE68A",
  },
  toolbarBtnMobileGreen: {
  ...
  toolbarBtnMobileTextFlash: {
    color: "#92400E",
  },
  toolbarBtnMobileTextGreen: {
    color: "#065F46",
  },
  ...
  sectionHeaderNew: { marginBottom: 10 },
  ...
  // Status Filter Chips
  statusFilterScroll: { marginBottom: 12 },
  statusFilterContent: { flexDirection: "row", gap: 8 },
  statusFilterChip: { flexDirection: "row", alignItems: "center", gap: 6, paddingVertical: 7, paddingHorizontal: 12, borderRadius: 20, backgroundColor: "#F8FAFC", borderWidth: 1.5, borderColor: "#E2E8F0" },
  statusFilterDot: { width: 6, height: 6, borderRadius: 3 },
  statusFilterLabel: { fontSize: 13, fontWeight: "600", color: "#475569" },
  statusFilterBadge: { paddingHorizontal: 7, paddingVertical: 2, borderRadius: 10, minWidth: 22, alignItems: "center" },
  statusFilterCount: { fontSize: 11, fontWeight: "800" },
  // Legacy status tab styles (kept for compatibility)
  statusTabs: { flexDirection: "row", backgroundColor: "#F8FAFC", borderRadius: 12, padding: 4, gap: 2 },
  statusTab: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 8, paddingHorizontal: 8, borderRadius: 10 },
  ...
  // Bulk Actions Bar
  bulkActionsBar: { flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 10, paddingVertical: 8, paddingHorizontal: 10, backgroundColor: "#F8FAFC", borderRadius: 10 },
  ...
  // Students Grid
  studentsGrid: { gap: 6 },
  studentCard: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", padding: 10, borderRadius: 12, borderWidth: 1, borderColor: "#E2E8F0", gap: 10 },
  studentCardSelected: { backgroundColor: "#F3E8FF", borderColor: colors.purple },
  studentCardDesktop: { maxWidth: "100%" },
  studentCardCheckbox: { marginRight: 2 },
  studentCardAvatar: { width: 38, height: 38, borderRadius: 19, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  studentCardAvatarText: { fontSize: 15, fontWeight: "700" },
  studentCardInfo: { flex: 1, minWidth: 0 },
  studentCardName: { fontSize: 14, fontWeight: "600", color: "#1E293B", marginBottom: 2 },
  studentCardMeta: { flexDirection: "row", alignItems: "center", gap: 6 },
  statusIndicator: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 7, paddingVertical: 2, borderRadius: 6 },
  statusIndicatorText: { fontSize: 11, fontWeight: "600" },
  studentCardValues: { alignItems: "flex-end", minWidth: 64 },
  studentCardAmount: { fontSize: 14, fontWeight: "800" },
  studentCardAmountLabel: { fontSize: 11, color: "#94A3B8", marginTop: 1 },
  ...
  modal: { backgroundColor: "#fff", borderRadius: 16, padding: 20, width: "100%", maxWidth: 400, maxHeight: "85%", overflow: "hidden" },
  createInvoiceModal: { maxHeight: "85%", maxWidth: 420, display: "flex", flexDirection: "column", padding: 24 } as any,
  modalInnerContainer: { flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" } as any,
  
  // ============ MODAL CRIAR COBRANÇA ============
  newInvoiceOverlay: { flex: 1, backgroundColor: "rgba(15, 23, 42, 0.6)", justifyContent: "center", alignItems: "center", padding: 16 },
  newInvoiceBackdrop: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0 } as any,
  newInvoiceContainer: { backgroundColor: "#fff", borderRadius: 18, width: "100%", maxWidth: 440, shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 24, elevation: 12, overflow: "hidden" } as any,

  // Compact header (ni = new invoice)
  niHeader: { flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 16, paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: "#F1F5F9" },
  niHeaderIcon: { width: 30, height: 30, borderRadius: 8, backgroundColor: "#F3E8FF", alignItems: "center", justifyContent: "center" },
  niTitle: { flex: 1, fontSize: 15, fontWeight: "700", color: "#1E293B" },
  niCloseBtn: { width: 28, height: 28, borderRadius: 8, backgroundColor: "#F8FAFC", alignItems: "center", justifyContent: "center" },

  // Body
  niBody: { paddingHorizontal: 14, paddingTop: 12, paddingBottom: 4 },
  niSection: { marginBottom: 12 },
  niSectionHeaderRow: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 },
  niSectionTitle: { fontSize: 11, fontWeight: "800", color: "#64748B", letterSpacing: 0.6, textTransform: "uppercase", marginBottom: 8 },

  // Tipo
  niTypeRow: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
  niTypeBtn: { flex: 1, minWidth: "45%", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 8, paddingHorizontal: 10, borderRadius: 10, backgroundColor: "#F8FAFC", borderWidth: 1.5, borderColor: "#E2E8F0" },
  niTypeBtnActive: { backgroundColor: colors.purple, borderColor: colors.purple },
  niTypeBtnText: { fontSize: 12, fontWeight: "600", color: "#64748B" },
  niTypeBtnTextActive: { color: "#fff" },

  // Filtros de gênero
  niFilterRow: { flexDirection: "row", gap: 5 },
  niFilterBtn: { paddingVertical: 4, paddingHorizontal: 10, borderRadius: 14, backgroundColor: "#F1F5F9" },
  niFilterBtnActive: { backgroundColor: "#E9D5FF" },
  niFilterText: { fontSize: 12, fontWeight: "600", color: "#64748B" },
  niFilterTextActive: { color: colors.purple },

  // Selecionar todos
  niSelectAllBtn: { flexDirection: "row", alignItems: "center", gap: 4, marginLeft: "auto" as any },
  niSelectAllText: { fontSize: 12, fontWeight: "600", color: colors.purple },

  // Chips de alunos (área com scroll interno)
  niChipScroll: { maxHeight: 96, backgroundColor: "#F8FAFC", borderRadius: 10 },
  niChips: { flexDirection: "row", flexWrap: "wrap", gap: 5, padding: 10, minHeight: 40 },
  niStudentChip: { flexDirection: "row", alignItems: "center", gap: 5, paddingVertical: 5, paddingHorizontal: 9, borderRadius: 14, backgroundColor: "#fff", borderWidth: 1.5, borderColor: "#E2E8F0" },
  niStudentChipSelected: { backgroundColor: colors.purple, borderColor: colors.purple },
  niStudentName: { fontSize: 12, fontWeight: "600", color: "#475569" },
  niStudentNameSelected: { color: "#fff" },
  niEmptyText: { fontSize: 12, color: "#94A3B8", fontStyle: "italic", textAlign: "center", width: "100%", paddingVertical: 4 },
  niSelectedCount: { fontSize: 11, fontWeight: "600", color: colors.purple, marginTop: 5, textAlign: "right" },

  // Campos de detalhe
  niFieldsRow: { flexDirection: "row", gap: 10, marginBottom: 10 },
  niField: { flex: 1 },
  niFieldLabel: { fontSize: 12, fontWeight: "600", color: "#64748B", marginBottom: 6 },
  niTextInput: { backgroundColor: "#F8FAFC", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, color: "#1E293B", borderWidth: 1.5, borderColor: "#E2E8F0" } as any,
  niValueBox: { flexDirection: "row", alignItems: "center", backgroundColor: "#F8FAFC", borderRadius: 10, borderWidth: 1.5, borderColor: "#E2E8F0", overflow: "hidden" },
  niValuePrefix: { paddingLeft: 12, paddingVertical: 10, fontSize: 14, fontWeight: "700", color: "#64748B" },
  niValueInput: { flex: 1, paddingLeft: 4, paddingRight: 12, paddingVertical: 10, fontSize: 14, color: "#1E293B", fontWeight: "600" } as any,

  // Footer
  niFooter: { flexDirection: "row", gap: 10, paddingHorizontal: 14, paddingVertical: 12, borderTopWidth: 1, borderTopColor: "#F1F5F9", backgroundColor: "#FAFBFC" },
  niCancelBtn: { flex: 1, paddingVertical: 11, borderRadius: 10, backgroundColor: "#F1F5F9", alignItems: "center", justifyContent: "center" },
  niCancelText: { fontSize: 14, fontWeight: "600", color: "#64748B" },
  niSubmitBtn: { flex: 1.5, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 11, borderRadius: 10, backgroundColor: colors.purple },
  niSubmitText: { fontSize: 14, fontWeight: "700", color: "#fff" },

  // Legacy newInvoice styles (kept for compatibility)
  newInvoiceHeader: { flexDirection: "row", alignItems: "center", paddingHorizontal: 24, paddingTop: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: "#F1F5F9" },
  newInvoiceHeaderIcon: { width: 40, height: 40, borderRadius: 12, backgroundColor: "#F3E8FF", alignItems: "center", justifyContent: "center", marginRight: 12 },
  newInvoiceTitle: { flex: 1, fontSize: 18, fontWeight: "700", color: "#1E293B" },
  newInvoiceCloseBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: "#F8FAFC", alignItems: "center", justifyContent: "center" },
  newInvoiceScroll: { flex: 1 },
  newInvoiceScrollContent: { padding: 24, paddingTop: 16 },
  newInvoiceSection: { marginBottom: 24 },
  newInvoiceSectionHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 },
  newInvoiceSectionTitle: { fontSize: 14, fontWeight: "700", color: "#334155", letterSpacing: 0.3 },
  newInvoiceTypeRow: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  newInvoiceTypeBtn: { flex: 1, minWidth: "45%", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 12, paddingHorizontal: 14, borderRadius: 12, backgroundColor: "#F8FAFC", borderWidth: 2, borderColor: "#E2E8F0" },
  newInvoiceTypeBtnActive: { backgroundColor: colors.purple, borderColor: colors.purple },
  newInvoiceTypeBtnText: { fontSize: 13, fontWeight: "600", color: "#64748B" },
  newInvoiceTypeBtnTextActive: { color: "#fff" },
  newInvoiceFilterRow: { flexDirection: "row", gap: 8, marginBottom: 12 },
  newInvoiceFilterBtn: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 20, backgroundColor: "#F1F5F9" },
  newInvoiceFilterBtnActive: { backgroundColor: "#E9D5FF" },
  newInvoiceFilterText: { fontSize: 13, fontWeight: "600", color: "#64748B" },
  newInvoiceFilterTextActive: { color: colors.purple },
  newInvoiceSelectAllBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 16, backgroundColor: "#F3E8FF" },
  newInvoiceSelectAllText: { fontSize: 13, fontWeight: "600", color: colors.purple },
  newInvoiceStudentsList: { flexDirection: "row", flexWrap: "wrap", gap: 8, padding: 16, backgroundColor: "#F8FAFC", borderRadius: 14, minHeight: 60 },
  newInvoiceStudentChip: { flexDirection: "row", alignItems: "center", gap: 6, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 20, backgroundColor: "#fff", borderWidth: 1.5, borderColor: "#E2E8F0" },
  newInvoiceStudentChipSelected: { backgroundColor: colors.purple, borderColor: colors.purple },
  newInvoiceStudentName: { fontSize: 13, fontWeight: "600", color: "#475569" },
  newInvoiceStudentNameSelected: { color: "#fff" },
  newInvoiceEmptyText: { fontSize: 13, color: "#94A3B8", fontStyle: "italic", textAlign: "center", width: "100%", paddingVertical: 8 },
  newInvoiceSelectedCount: { fontSize: 12, fontWeight: "600", color: colors.purple, marginTop: 10, textAlign: "right" },
  newInvoiceFieldsGrid: { flexDirection: "row", gap: 12, marginBottom: 16 },
  newInvoiceField: { flex: 1 },
  newInvoiceFieldFull: { marginBottom: 8 },
  newInvoiceFieldLabel: { fontSize: 13, fontWeight: "600", color: "#64748B", marginBottom: 8 },
  newInvoiceTextInput: { backgroundColor: "#F8FAFC", borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, fontSize: 15, color: "#1E293B", borderWidth: 1.5, borderColor: "#E2E8F0" } as any,
  newInvoiceValueBox: { flexDirection: "row", alignItems: "center", backgroundColor: "#F8FAFC", borderRadius: 12, borderWidth: 1.5, borderColor: "#E2E8F0", overflow: "hidden" },
  newInvoiceValuePrefix: { paddingLeft: 16, paddingVertical: 14, fontSize: 15, fontWeight: "700", color: "#64748B" },
  newInvoiceValueInput: { flex: 1, paddingLeft: 4, paddingRight: 16, paddingVertical: 14, fontSize: 15, color: "#1E293B", fontWeight: "600" } as any,
  newInvoiceFooter: { flexDirection: "row", gap: 12, paddingHorizontal: 24, paddingVertical: 20, borderTopWidth: 1, borderTopColor: "#F1F5F9", backgroundColor: "#FAFBFC" },
  newInvoiceCancelBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, backgroundColor: "#F1F5F9", alignItems: "center", justifyContent: "center" },
  newInvoiceCancelText: { fontSize: 15, fontWeight: "600", color: "#64748B" },
  newInvoiceSubmitBtn: { flex: 1.5, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 14, borderRadius: 12, backgroundColor: colors.purple },
  newInvoiceSubmitText: { fontSize: 15, fontWeight: "700", color: "#fff" },
  settingsModal: { maxWidth: 500, maxHeight: "90%", padding: 0, display: "flex", flexDirection: "column" } as any,
  settingsModalScroll: { flex: 1 },
  settingsModalScrollContent: { padding: 16, paddingBottom: 24 },
  ...
  pixCode: { fontSize: 11, fontFamily: "monospace", color: "#1E293B", lineHeight: 18 },
  batchInfo: { fontSize: 14, color: "#64748B", textAlign: "center", marginBottom: 16, lineHeight: 20 },
  modalBtns: { flexDirection: "row", gap: 12, marginTop: 20, paddingTop: 16, borderTopWidth: 1, borderTopColor: "#E2E8F0" },
  ...
  previewPricingHeader: {
    flexDirection: "row",
    backgroundColor: "#E2E8F0",
    paddingVertical: 10,
    paddingHorizontal: 12,
  },
  previewPricingHeaderText: {
    flex: 1,
    fontSize: 12,
    fontWeight: "700",
    color: "#475569",
  ...
  previewStudentAmount: {
    alignItems: "flex-end",
  },
  previewStudentAmountValue: {
    fontSize: 16,
    fontWeight: "800",
  },
  previewStudentAmountOriginal: {
    fontSize: 12,
    color: "#94A3B8",
  ...
  vmScroll: { flex: 1 },
  vmScrollContent: { paddingHorizontal: 14, paddingTop: 12, paddingBottom: 4 },
  vmSectionTitle: {
    fontSize: 11,
    fontWeight: "700",
    color: "#94A3B8",
